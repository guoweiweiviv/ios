...
- (void)viewDidLoad
{
    ...
    if (self.immediateReturn) {
        ...
        doneButtonItem.tintColor = [UIColor colorWithRed:51.0f/255.0f green:51.0f/255.0f blue:51.0f/255.0f alpha:1.0];
...
- (void)preparePhotos
{
    @autoreleasepool {
      ...
      PHFetchResult *rst = [self fetchAssetsInAssetCollection:self.assetGroup ascending:NO];
        [rst enumerateObjectsUsingBlock:^(PHAsset *result, NSUInteger index, BOOL *stop) {
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
          ...
          //  [self setTitle:self.singleSelection ? @"选择图片" : @"选择图片" color:nil];
        });
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
  CGSize size = [[UIScreen mainScreen] bounds].size;
	return (size.width / self.columns ) -1;
}
